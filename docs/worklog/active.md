...
- Gate 3.2i short-budget boundary-index localizer completed as a
  deployable-negative result: the window-level close/open index objective
  improves only trivially over Gate 3.2h's best predicted mask
  (`0.035053` vs `0.035201`) and remains worse than the Gate 3.1f/Gate 3.1g
  reference `0.034767`. Within the same 3.2i checkpoints, the boundary-index
  readout worsens base action MSE (`0.034878 -> 0.035053`) and transition MSE
  (`0.134422 -> 0.135682`), while exact event-window localization remains weak
  (`close=0.069527`, `open=0.023539`). Treat short-budget option A as
  exhausted; pivot next to a richer temporal/flow action decoder or direct
  gripper-transition trajectory model.
- Gate 3.3 minimal temporal gripper trajectory decoder completed as a
  deployable-negative short-budget result:
  plan archived at
  `docs/experiments/plans/2026-06-15_gate3_3_minimal_temporal_gripper_trajectory_decoder_plan.md`.
  The optional `temporal_mlp` gripper residual sequence branch predicts `[B,H]`
  gripper residuals and reports `trajectory_routed_*` metrics while keeping the
  Gate 3.1f/g predicted top-4 event/rank/prob sample interface fixed. The
  branch passes plumbing, repeated eval, and group-audit execution, but worsens
  the same checkpoint's base output on the two-seed short run:
  overall MSE `0.035516 -> 0.035655`, transition MSE
  `0.137769 -> 0.138046`, and gripper MSE `0.153991 -> 0.154966`.
  Do not expand controls for this shallow additive residual; keep Gate 3.1f/g
  as the deployable reference.
- Gate 3.4 controlled joint temporal action-sequence decoder completed as a
  short-budget mechanism-positive but attribution-cautious result:
  plan archived at
  `docs/experiments/plans/2026-06-15_gate3_4_controlled_joint_temporal_action_sequence_decoder_plan.md`,
  formal run record at
  `docs/experiments/runs/2026-06-15_gate3_4_controlled_joint_temporal_action_sequence_decoder.md`,
  and decision summary at
  `docs/experiments/comparisons/2026-06-15_gate3_4_controlled_joint_temporal_decoder_summary.md`.
  The optional `sequence_mlp` branch predicts full `[B,H,A]`
  `temporal_actions` while preserving the base action output. Mean repeated
  eval over seeds 7/17 improves full aligned overall MSE from same-checkpoint
  base `0.034303` to temporal `0.034262`, and transition MSE from `0.131697`
  to `0.131311`, beating the Gate 3.1f/g reference `0.034767` / `0.134087`.
  Controls remain necessary and informative: full aligned temporal
  `0.034262` beats shuffled event `0.035529`, rank/prob-only `0.035875`, and
  context-only/no-prior `0.036642`, but mean replacement is close at
  `0.034414`. Treat Gate 3.4 as a small positive for joint temporal decoding
  and aligned event metadata, not yet as strong proof of full K-sample
  diversity use.
...
- Gate 3.4b sample-diversity usage audit completed. Same-checkpoint eval-time
  mean collapse hurts full-aligned Gate 3.4 (`0.034218 -> 0.042594` temporal
  MSE), while permutation is invariant and batch mismatch fails, so the trained
  full checkpoint uses K-sample distribution at runtime. The separately trained
  mean-only control remains close, so the next target is to make diversity use
  stronger under matched training controls.
- Gate 3.4c sample-score temporal regret supervision completed as a
  negative/neutral short-budget result:
  plan archived at
  `docs/experiments/plans/2026-06-16_gate3_4c_sample_score_temporal_regret_plan.md`,
  run record at
  `docs/experiments/runs/2026-06-16_gate3_4c_sample_score_temporal_regret.md`,
  and decision summary at
  `docs/experiments/comparisons/2026-06-16_gate3_4c_sample_score_temporal_regret_summary.md`.
  The optional `sample_score_mode=action_regret` branch scores each K-sample
  future-motion token with a motion-regret soft-CE auxiliary loss, then uses
  the score softmax for set aggregation. Mean repeated eval over seeds 7/17
  reaches temporal-action MSE `0.034779`, gripper MSE `0.154203`, and
  transition MSE `0.141620`, which is worse than Gate 3.4 full aligned
  (`0.034262` / `0.149383` / `0.131311`). Do not expand the full Gate 3.4c
  control matrix; keep the scorer as a diagnostic hook only. The next mainline
  should test a small flow/diffusion residual decoder with the same attribution
  controls: decoder gain, prior gain, metadata gain, diversity gain,
  eval-time mean collapse, permutation, and batch mismatch.
- Gate 3.5a small residual flow decoder completed as a negative short-budget
  result:
  plan archived at
  `docs/experiments/plans/2026-06-16_gate3_5a_small_residual_flow_decoder_plan.md`,
  run record at
  `docs/experiments/runs/2026-06-16_gate3_5a_small_residual_flow_decoder.md`,
  and decision summary at
  `docs/experiments/comparisons/2026-06-16_gate3_5a_small_residual_flow_decoder_summary.md`.
  The optional `flow_action_decoder_mode=rectified_mlp` branch predicts a
  residual after `temporal_actions`, with a deployable `flow_actions` readout
  plus a rectified-flow velocity auxiliary loss. Mean 5-pass repeated eval over
  seeds 7/17 gives Gate 3.5a temporal MSE `0.035392` and flow MSE `0.035621`,
  both worse than Gate 3.4 temporal MSE `0.034262`. Same-checkpoint decoder
  gain is negative (`0.035392 - 0.035621 = -0.000229`), and transition MSE
  regresses (`0.136396 -> 0.137830`, vs Gate 3.4 `0.131311`). Do not run the
  full controls. The next clean branch is a post-hoc residual adapter over a
  frozen Gate 3.4 checkpoint, to test residual action modeling without letting
  the new objective degrade the temporal decoder.
- Gate 3.11a GraspGen-style on-generator GeoMoCo critic audit is planned and
  minimally implemented:
  plan archived at
  `docs/experiments/plans/2026-07-06_gate3_11a_on_generator_geomoco_critic_audit_plan.md`,
  implementation in `scripts/train_on_generator_geomoco_critic.py`, and tests
  in `tests/test_on_generator_geomoco_critic.py`. The script freezes the Gate
  3.4 temporal action head, labels current GeoMoCo generator candidates by
  single-candidate temporal-action regret, and trains a lightweight critic to
  rank those on-generator candidates. CPU dry-run and one-batch smoke passed;
  formal seed-7 real/control training remains to run.
- Gate 3.11a seed-7 real on-generator critic branch completed as a
  candidate-space-positive but critic-extraction-negative result. The generated
  candidates contain a much better oracle choice than the Gate 3.4 set readout
  (`candidate_oracle_mse=0.026081` vs `set_temporal_mse=0.036371`), especially
  on transition windows (`0.066808` vs `0.137058`), but the learned critic
  selects worse candidates (`critic_selected_mse=0.050388`, transition
  `0.188597`). Do not expand the broad control matrix until the critic target is
  redesigned.
...
- 2026-06-15 Gate 3.2i boundary-index localizer completed:
  - formal record:
    `docs/experiments/runs/2026-06-15_gate3_2i_boundary_index_localizer.md`;
  - comparison:
    `docs/experiments/comparisons/2026-06-15_gate3_2i_short_budget_boundary_localizer_summary.md`;
  - code:
    `src/geomoco_wm/models/motion_prior_action_head.py`,
    `scripts/train_predicted_event_mixture_action_head.py`,
    `scripts/evaluate_predicted_event_mixture_action_head.py`,
  ...
  - artifacts:
    ...
    `outputs/motion_prior_action_head/gate3_2i_boundary_index_top4_k16_seed{7,17}/`;
  - mean repeated-eval result:
    base overall MSE `0.034878`, boundary-index readout MSE `0.035053`,
    boundary-index gripper MSE `0.152685`, boundary-index transition MSE
    `0.135682`;
  - localization:
    close exact `0.069527`, open exact `0.023539`, close within-1 `0.249839`,
    open within-1 `0.160918`;
  - interpretation: the index objective is a genuinely different localization
    target, but still fails to recover the Gate 3.2h oracle-mask gain. It
    slightly improves over the best Gate 3.2h predicted mask but does not beat
    Gate 3.1f/Gate 3.1g and does not improve over its own base output. Option
    A's short-budget deterministic boundary-localization branch is exhausted.
- 2026-06-15 Gate 3.3 minimal temporal gripper trajectory decoder completed:
  - plan:
    `docs/experiments/plans/2026-06-15_gate3_3_minimal_temporal_gripper_trajectory_decoder_plan.md`;
  - formal record:
    `docs/experiments/runs/2026-06-15_gate3_3_minimal_temporal_gripper_trajectory_decoder.md`;
  - comparison:
    `docs/experiments/comparisons/2026-06-15_gate3_3_minimal_temporal_gripper_trajectory_summary.md`;
  - code:
    `src/geomoco_wm/models/motion_prior_action_head.py`,
    `scripts/train_predicted_event_mixture_action_head.py`,
    `scripts/evaluate_predicted_event_mixture_action_head.py`,
  ...
  - artifacts:
    ...
    `outputs/motion_prior_action_head/gate3_3_gripper_traj_top4_k16_seed{7,17}/`;
  - mean repeated-eval result:
    base overall MSE `0.035516`, trajectory-routed MSE `0.035655`,
    base transition MSE `0.137769`, trajectory-routed transition MSE
    `0.138046`, base gripper MSE `0.153991`, trajectory-routed gripper MSE
    `0.154966`;
  - interpretation: the minimal temporal residual branch is plumbing-positive
    but metric-negative. It does not beat the Gate 3.1f/g reference and does
    not even improve over the same checkpoint's base output, so attribution
    controls are not expanded for this branch.
- 2026-06-15 Gate 3.4 controlled joint temporal action-sequence decoder
  completed:
  - plan:
    `docs/experiments/plans/2026-06-15_gate3_4_controlled_joint_temporal_action_sequence_decoder_plan.md`;
  - formal record:
  ...
  - code:
    ...
    `tests/test_motion_prior_action_head.py`;
  - artifacts:
    `outputs/motion_prior_action_head/gate3_4_temporal_action_top4_k16_seed{7,17}/`,
    `outputs/motion_prior_action_head/gate3_4_temporal_action_shuffled_event_top4_k16_seed{7,17}/`,
    `outputs/motion_prior_action_head/gate3_4_temporal_action_rankprob_top4_k16_seed{7,17}/`,
    `outputs/motion_prior_action_head/gate3_4_temporal_action_mean_repeated_top4_k16_seed{7,17}/`,
    `outputs/motion_prior_action_head/gate3_4_temporal_action_context_only_seed{7,17}/`;
  - mean repeated-eval result:
    full aligned base MSE `0.034303`, full aligned temporal-action MSE
    `0.034262`, temporal gripper MSE `0.149383`, temporal transition MSE
    `0.131311`, temporal sustain MSE `0.022542`;
  - controls using temporal actions:
    shuffled event MSE `0.035529`, rank/prob-only MSE `0.035875`,
    mean repeated MSE `0.034414`, context-only/no-prior MSE `0.036642`;
  - interpretation: joint temporal decoding is a controlled small positive and
    same-capacity no-prior decoding is worse, but mean replacement remains
    close. The safe claim is that aligned event metadata plus motion-prior
    mean/sample structure remains useful; stronger sample-diversity usage is
    still unresolved.
...
- Gate 3.4b clarifies the sample-diversity attribution story. The full-aligned
  temporal decoder does use K-sample structure at runtime: eval-time
  `mean_repeated` raises temporal MSE from `0.034218` to `0.042594`, with larger
  damage on transition, high-diversity, and high best-vs-mean-gap groups. But
  the separately trained mean-only Gate 3.4 control remained close to full
  aligned (`0.034414` vs `0.034262`), so the next mechanism question is not
  whether samples are ignored; it is whether training can make sample diversity
  causally necessary under the same controls.
- Gate 3.4c answers the first candidate-comparison attempt negatively:
  motion-regret sample scoring does not improve action value (`0.034779`
  temporal MSE vs Gate 3.4 `0.034262`) and worsens transition MSE
  (`0.141620` vs `0.131311`). The likely issue is target mismatch: future-motion
  closeness is not the same as downstream gripper-transition/action regret.
  This keeps the diversity-attribution problem open, but suggests the next
  mainline should change the residual action trajectory model itself rather
  than add another motion-regret scorer.
- Gate 3.5a shows that simply joint-training a tiny rectified-flow residual
  after `temporal_actions` is also not enough. The residual readout is worse
  than the same checkpoint's temporal readout, and the temporal readout itself
  regresses from Gate 3.4. Treat this as evidence against shared joint
  residual-flow training, not against all residual adapters. The next fair test
  is to freeze Gate 3.4 and train a small post-hoc adapter.
- Gate 3.5b frozen post-hoc residual adapter completed as a mixed result:
  discussion archived at
  `docs/agent_qa/2026-06-16-gate35b-posthoc-adapter-motivation.md`,
  plan at
  `docs/experiments/plans/2026-06-16_gate3_5b_frozen_posthoc_residual_adapter_plan.md`,
  run record at
  `docs/experiments/runs/2026-06-16_gate3_5b_frozen_posthoc_residual_adapter.md`,
  and summary at
  `docs/experiments/comparisons/2026-06-16_gate3_5b_posthoc_residual_adapter_summary.md`.
  It improves overall MSE versus Gate 3.4 (`0.033374` vs `0.034262`) and
  full aligned remains best among matched trained adapter controls, but
  attribution is thin: context-only reaches `0.033728`, prior gain is only
  `+0.000354`, and diversity gain is `+0.000230`. The branch does not solve
  the main transition bottleneck: full-aligned transition MSE is `0.133339`,
  worse than Gate 3.4 `0.131311` and context-only `0.131042`.
- Gate 3.5c transition-constrained post-hoc residual completed as a useful
  negative:
  plan at
  `docs/experiments/plans/2026-06-16_gate3_5c_transition_constrained_posthoc_residual_plan.md`,
  run record at
  `docs/experiments/runs/2026-06-16_gate3_5c_transition_constrained_posthoc_residual.md`,
  and summary at
  `docs/experiments/comparisons/2026-06-16_gate3_5c_transition_constrained_posthoc_residual_summary.md`.
  The deployable predicted-transition gate preserves a small overall gain
  (`0.033810` vs Gate 3.4 `0.034262`) but fails the transition promotion check:
  transition MSE remains worse than Gate 3.4 (`0.133308` vs `0.131311`) and
  the same-checkpoint transition decoder gain is negative (`-0.002392`). Do
  not spend full controls on this failed branch; pivot next to upstream
  event/transition candidate quality while keeping the attribution ledger.
- Gate 3.6a upstream transition-reserve candidate quality completed, then
  post-hoc corrected by Gate 3.6b/3.6c:
  plan at
  `docs/experiments/plans/2026-06-17_gate3_6a_upstream_transition_candidate_quality_plan.md`,
  run record at
  `docs/experiments/runs/2026-06-17_gate3_6a_transition_reserve_candidate_quality.md`,
  and summary at
  `docs/experiments/comparisons/2026-06-17_gate3_6a_transition_reserve_candidate_quality_summary.md`.
- Gate 3.6b Pareto transition candidate allocation sweep completed:
  run record at
  `docs/experiments/runs/2026-06-17_gate3_6b_pareto_transition_candidate_allocation.md`
  and candidate-vs-selection summary at
  `docs/experiments/comparisons/2026-06-17_gate3_6b_3_6c_candidate_vs_selection_attribution_summary.md`.
  Conservative `transition_reserve` thresholds 0.25/0.35 with
  `selection_metric=temporal_action_mse` exactly match Gate 3.4
  (`0.034262` temporal, `0.131311` transition, `0.022542` sustain).
  Trigger diagnostics show why: top-4 already contains a transition candidate
  for every train/validation window, so `transition_reserve` triggers zero
  replacements at thresholds 0.05/0.10/0.15/0.25/0.35.
- Gate 3.6c top-k transition-selection attribution check completed:
  run record at
  `docs/experiments/runs/2026-06-17_gate3_6c_topk_transition_selection_attribution.md`.
  Top-k candidates with only `selection_metric=temporal_action_transition_mse`
  exactly reproduce Gate 3.6a (`0.036391` temporal, `0.126421` transition,
  `0.025539` sustain). Corrected interpretation: the transition gain is real
  but comes from transition-MSE checkpoint selection / early stopping, not from
  candidate reserve replacement. Do not continue fixed-threshold reserve tuning
  at top-4 unless the candidate policy actually changes the selected set.
- 2026-06-20 event-time latent vs action-chunk policy discussion archived at
  `docs/agent_qa/2026-06-20-event-time-latent-vs-action-chunk-policy.md`.
  Decision: try Gate 3.7a first as a minimal soft event-time latent +
  conditional temporal decoder, instead of jumping immediately to a stronger
  motion-prior-conditioned diffusion/action-chunk policy.
- Gate 3.7a plan archived at
  `docs/experiments/plans/2026-06-20_gate3_7a_soft_event_time_latent_conditional_decoder_plan.md`.
  It will predict a soft close/open event-time distribution and feed that
  distribution into the temporal action decoder as continuous conditioning.
  The key guardrail is to avoid hard argmax boundary routing; earlier hard
  boundary-index/localizer variants already failed.
- Gate 3.7a plumbing implemented and smoke-tested:
  `MotionPriorActionHead` now supports
  `event_time_conditioning_mode=soft_boundary`, which predicts close/open
  distributions over `H+1` classes and injects the soft distribution into the
  `sequence_mlp` temporal decoder. Training/eval scripts now report
  `event_time_*` CE/accuracy/within-1/entropy diagnostics. CPU smoke artifact:
  `outputs/motion_prior_action_head/gate3_7a_soft_event_time_smoke_seed7/`.
  Verification passed: compileall, ruff, and 69 unittest cases.
- Gate 3.7a seed-7 short-budget full-aligned run completed as a negative:
  run record at
  `docs/experiments/runs/2026-06-20_gate3_7a_soft_event_time_latent_short_budget.md`.
  The branch failed the pre-declared expansion gate, with 5-pass repeated eval
  `temporal_action_mse=0.037941`, transition MSE `0.147882`, sustain MSE
  `0.023835`, and gripper MSE `0.171543`, all worse than the Gate 3.4 seed-7
  full-aligned reference (`0.036502` temporal, `0.137827` transition,
  `0.023501` sustain, `0.164389` gripper). Event-time diagnostics show why
  this minimal variant is weak: overall event-time accuracy is high
  (`0.937472`) because no-event labels dominate, but positive timing remains
  poor (`close within-1=0.350704`, `open within-1=0.156866`). Stop this
  Gate 3.7a variant under the short-budget instruction; do not run seed 17 or
  controls unless the event-time objective is redesigned first.
- Gate 3.8a controlled small action-chunk decoder completed as a short-budget
  negative:
  plan archived at
  `docs/experiments/plans/2026-06-20_gate3_8a_controlled_small_action_chunk_decoder_plan.md`
  and run record at
  `docs/experiments/runs/2026-06-20_gate3_8a_controlled_small_action_chunk_decoder.md`.
  The new `temporal_action_decoder_mode=temporal_transformer` keeps the Gate
  3.4 predicted top-4 event/rank/prob sample interface fixed, reuses the
  `temporal_actions [B,H,A]` output/loss, and only adds step-token temporal
  self-attention before per-step action prediction. Seed-7 full-aligned
  repeated eval fails the stop rule: temporal MSE `0.038275` vs Gate 3.4 seed-7
  `0.036502`, transition MSE `0.162345` vs `0.137827`, and gripper MSE
  `0.176774` vs `0.164389`; sustain improves (`0.022356` vs `0.023501`) but
  at the cost of much worse transition/gripper timing. Stop this branch at
  seed 7; do not run seed 17 or controls.
- 2026-06-20 strong-policy replacement risk discussion archived at
  `docs/agent_qa/2026-06-20-strong-policy-replacement-and-flow-matching.md`.
  The core framing is now explicit: a strong direct action policy may replace
  GeoMoCo-WM's in-distribution action-MSE benefit, so the right next audit is
  not another small timing/readout repair but a matched strong-policy
  replacement comparison.
- Gate 3.9a flow-matching replacement audit plan archived at
  `docs/experiments/plans/2026-06-20_gate3_9a_flow_matching_replacement_audit_plan.md`,
  with initial run record at
  `docs/experiments/runs/2026-06-20_gate3_9a_flow_matching_replacement_audit_initial.md`.
  Implemented `scripts/train_flow_matching_action_policy.py`, a conditional
  rectified-flow action-chunk policy with `direct_visual` and `geomoco`
  condition modes. The seed-7 initial pair is mixed: GeoMoCo-conditioned flow
  improves overall MSE (`0.059032 -> 0.058218`) and transition MSE
  (`0.227686 -> 0.216756`) versus direct visual flow, but slightly worsens
  gripper MSE (`0.266500 -> 0.267212`) and both branches are much weaker than
  Gate 3.4 (`0.036502` temporal MSE, `0.137827` transition MSE). Do not expand
  to seed17/full controls yet; first calibrate the flow policy so direct visual
  flow is a credible strong replacement baseline.
- Gate 3.9a flow calibration and dataset sufficiency audit completed:
  run record at
  `docs/experiments/runs/2026-06-20_gate3_9a_flow_calibration_and_dataset_sufficiency.md`.
  Added eval-only support to `scripts/train_flow_matching_action_policy.py`
  and added `scripts/audit_dataset_sufficiency.py`. Increasing Euler steps
  from 8 to 16/32 worsens both direct and GeoMoCo flow branches, so the weak
  flow result is not due to too few integration steps. The data audit confirms
  the user's concern: the current 2-files-per-suite slice has `16,518` windows
  but only `1,699` transition windows (`10.29%`), with close transitions `878`
  and open transitions `807`; one task,
  `open_the_middle_drawer_of_the_cabinet`, has zero transition labels under the
  current event definition. Do not continue full strong-policy controls on this
  slice until training data are expanded or transition-balanced.
- Gate 3.10a full LIBERO data reliability audit completed:
  plan at
  `docs/experiments/plans/2026-06-20_gate3_10_data_reliability_first_plan.md`
  and run record at
  `docs/experiments/runs/2026-06-20_gate3_10a_full_libero_data_reliability_audit.md`.
  The full local LIBERO export covers `40` HDF5 task files, `2,000` episodes,
  and `80,883` windows with no export warnings. Full-data event coverage is now
  sufficient globally for the transition/open-close bottleneck:
  `9,148` transition windows (`11.31%`), `4,849` close transitions, and
  `4,246` open transitions. Caveats remain important: transition windows are
  still a minority, `mixed_transition` modes are too rare for separate training
  (`53` total, `mixed_transition::late=1`), and two tasks have zero transition
  labels under the current command-transition definition:
  `open_the_middle_drawer_of_the_cabinet` and
  `push_the_plate_to_the_front_of_the_stove`.
- Gate 3.10b per-window bootstrap CI audit completed:
  run record at
  `docs/experiments/runs/2026-06-21_gate3_10b_per_window_bootstrap_ci_audit.md`.
  Added reusable per-window metric helpers in
  `src/geomoco_wm/metrics/window_metrics.py` and extended Gate 3.4 / Gate 3.9
  evaluators with `--per-window-output-jsonl`. Existing 2-files checkpoints
  were audited by episode-bootstrap CI. Gate 3.4 seed 7/17 results are
  consistent: full-vs-context prior gain on `temporal_action_mse` is positive
  in mean but CI crosses zero, while `temporal_action_se3_mse` gain is stable;
  full-vs-mean diversity gain is tiny and unreliable; full-vs-shuffled/rankprob
  metadata gain is stable for overall `temporal_action_mse`. Transition-sliced
  prior gain still crosses zero. Gate 3.9 seed-7 flow audit shows reliable
  GeoMoCo SE(3) gain over direct visual flow, but no reliable overall action
  MSE gain and a reliable negative on transition-close action/gripper MSE.
- Gate 3.10c transition-balanced small training audit completed as a
  deployable-negative attribution result:
  run record at
  `docs/experiments/runs/2026-06-21_gate3_10c_transition_balanced_small_training_audit.md`.
  Added `--train-sampling-mode transition_balanced` and
  `--transition-sampling-fraction` to
  `scripts/train_predicted_event_mixture_action_head.py`, with validation kept
  on the natural episode split. The sampler raises train transition draw
  probability from the natural `0.100260` to target `0.500000`. Full-aligned
  transition MSE improves versus Gate 3.4 (`0.136756 -> 0.122302`) but
  matched controls break the attribution: context-only is better on transition
  MSE (`0.109508`), mean-repeated is better overall, and shuffled-event beats
  full aligned on transition close/open action/gripper CI. Episode-bootstrap CI
  shows full aligned is reliably worse than context-only on transition-close
  temporal action MSE and gripper MSE. Do not spend full-data DINO/cache/model
  budget as a blind scale-up from this branch.
- Gate 3.11a GraspGen-style on-generator GeoMoCo critic audit plumbing
  completed after the GraspGen design discussion:
  plan at
  `docs/experiments/plans/2026-07-06_gate3_11a_on_generator_geomoco_critic_audit_plan.md`.
  Added `scripts/train_on_generator_geomoco_critic.py`, which freezes a Gate
  3.4 temporal action-head checkpoint, samples predicted event-mixture GeoMoCo
  candidates, labels each candidate by frozen single-candidate
  temporal-action regret, and trains a small MLP critic. It reports
  `candidate_oracle_mse`, `oracle_gain_vs_set`, `critic_selected_mse`,
  `critic_selected_gain_vs_set`, `critic_gap_to_oracle`, top-1 ranking
  accuracy, and transition/sustain group metrics. Supported controls are
  `mean_repeated`, `rank_prob_only`, `shuffled_event_identity`,
  `batch_mismatch`, and `zero_sample_features`. Verification passed:
  `py_compile`, focused unittest, full `unittest discover -s tests` with
  156 tests, `ruff check`, real-checkpoint `--dry-run`, and a CPU one-batch
  smoke artifact at `/tmp/gate3_11a_smoke/`. Formal results are not available
  yet; do not interpret smoke metrics.
- Gate 3.11a seed-7 real branch completed:
  run record at
  `docs/experiments/runs/2026-07-06_gate3_11a_on_generator_geomoco_critic_real_seed7.md`,
  artifacts at
  `outputs/on_generator_geomoco_critic/gate3_11a_real_seed7/metrics.json` and
  `outputs/on_generator_geomoco_critic/gate3_11a_real_seed7/model.pt`. The
  restricted shell could not see CUDA, so the run used the approved
  GPU-visible execution mode. Result: the GeoMoCo candidate set has strong
  oracle headroom (`candidate_oracle_mse=0.026081` vs `set_temporal_mse=0.036371`,
  `oracle_gain_vs_set=0.010291`), with transition oracle headroom
  `0.070250` (`0.137058 -> 0.066808`). However, the first MLP critic fails to
  extract that value: `critic_selected_mse=0.050388`,
  `critic_selected_gain_vs_set=-0.014017`, top-1 accuracy `0.184819`, and
  transition selection regresses badly (`critic_selected_mse=0.188597` vs set
  `0.137058`). Stop broad controls for this exact critic; redesign target or
  scorer structure first.
...
- Full four-suite export/audit is now complete for `horizon=8`, `stride=4`.
  Gate 3.10c argues against blind full-data retraining: the immediate blocker
  is now identifying the missing signal for deployable open-close timing, not
  raw data volume alone.
- Gate 3.11a seed-7 real is complete. Formal controls are paused because the
  first critic failed the deployable selection check even though the candidate
  oracle headroom is positive.
...
1. Gate 3.6a/3.6b/3.6c are complete and not promoted as default. The
   transition-positive result is real (`0.126421` vs Gate 3.4 `0.131311`), but
   3.6b/3.6c correct its cause: it comes from
   `selection_metric=temporal_action_transition_mse`, not from
   `transition_reserve` candidate replacement.
2. Keep the immediate background: Gate 3.4b showed runtime K-sample usage,
   Gate 3.4c motion-regret scoring failed, Gate 3.5a joint residual-flow
   training regressed, Gate 3.5b post-hoc residual capacity helped overall but
   not transition, Gate 3.5c showed deployable transition gating still does not
   repair close/open timing, and Gate 3.6c showed transition-focused checkpoint
   selection creates the same overall/sustain trade-off as 3.6a.
3. Do not continue fixed-threshold `transition_reserve` tuning at top-4. It is
   inert on the current split because top-4 already always includes a
   transition candidate. Candidate work should only resume with a policy that
   actually changes selected candidates, such as transition-timing diversity,
   multiple transition slots, or downstream action/transition-regret scoring.
4. Gate 3.7a minimal soft event-time latent + conditional temporal decoder has
   now been tried on seed 7 and stopped early as a short-budget negative. It
   kept the Gate 3.4 top-k event/rank/prob sample interface and selected by
   `temporal_action_mse`, but worsened temporal/transition/sustain/gripper MSE
   and did not pass the expansion criteria. Treat this as evidence against the
   current unbalanced soft-boundary token design, not against every possible
   event-time latent.
5. Gate 3.8a controlled small action-chunk decoder also stopped at seed 7 as a
   short-budget negative. A tiny temporal-transformer readout slightly improves
   sustain but badly worsens transition/gripper MSE. Treat this as evidence
   against the cheapest `same features + same loss + temporal self-attention`
   decoder, not against stronger action-chunk policies in general.
6. Gate 3.9a initial flow-matching replacement audit is implemented and run for
   seed 7 direct-vs-GeoMoCo. GeoMoCo flow gives a weak transition/overall gain
   over direct visual flow, but gripper is slightly worse and both flow branches
   are far below Gate 3.4. The immediate next step is not full controls; it is
   flow-policy calibration, such as more Euler steps, deterministic action
   readout auxiliary calibration, or lower-variance evaluation, until
   `direct_visual_flow` is a credible strong policy baseline.
7. Flow eval-only calibration and the 2-files data audit showed that more
   Euler steps do not fix flow MSE and the old slice is too sparse for the
   bottleneck: only `10.29%` transition windows, `878` close transitions, and
   `807` open transitions.
8. Gate 3.10a full LIBERO audit now changes the data status: full data has
   `80,883` windows and passes the global transition sufficiency target with
   `9,148` transition windows, `4,849` close transitions, and `4,246` open
   transitions. Still report transition/sustain separately, keep mixed
   transitions diagnostic, and isolate zero-transition tasks in transition
   claims.
9. Gate 3.10b is complete on existing 2-files checkpoints. Per-window export
   now exists for Gate 3.4/Gate 3.9, and episode-bootstrap CI has changed the
   interpretation: metadata/SE(3) gains are reliable, but prior gain over
   context-only on action MSE and transition/open-close timing gains are not
   reliable; sample-diversity gain over mean_repeated is tiny and unreliable.
10. Gate 3.10c transition-balanced small training audit is complete and
   negative for GeoMoCo attribution. Transition-balanced sampling can improve
   absolute transition MSE, but matched controls show the gain is mainly a
   training-distribution/checkpoint-selection effect: context-only beats full
   aligned on transition MSE, and shuffled-event beats full aligned on
   transition close/open action/gripper CI.
11. Gate 3.11a seed-7 real creates a useful split diagnosis:
   candidate-space value is real (`set_temporal_mse - candidate_oracle_mse =
   0.010291`, transition `0.070250`), but critic extraction is negative
   (`set_temporal_mse - critic_selected_mse = -0.014017`, transition
   `-0.051539`). Do not run the broad `mean_repeated` / `rank_prob_only` /
   `shuffled_event_identity` / `batch_mismatch` / `zero_sample_features`
   control matrix for this exact MLP critic; it already fails the real-branch
   selection gate.
12. For Gate 3.11a, track:
   candidate-space value = `set_temporal_mse - candidate_oracle_mse`;
   critic extraction value = `set_temporal_mse - critic_selected_mse`;
   critic residual gap = `critic_selected_mse - candidate_oracle_mse`;
   metadata/control attribution = real minus rank/prob-only, shuffled-event,
   mean-repeated, batch-mismatch, and zero-feature controls.
13. The next 3.11 variant, if pursued, should redesign the critic before
   controls: transition-specific regret, gripper/contact regret, pairwise or
   set-wise candidate comparison, or contact/object-state/task-phase proxy
   labels. Gate 3.10d label/contact audit remains the fallback if those labels
   are unavailable.
14. Preserve the attribution ledger for any next positive decoder or candidate
   branch:
...
15. Do not mix Gate 3 action-head metrics with old frozen-action-decoder metrics;
   use them as separate evidence streams.
16. For heavy training, use a GPU-visible shell or approved execution mode; the
