...
- Next run target: add visual grounding and/or action-aware supervision so the
  learned prior can move between the direct-context lower bound and
  oracle-future-motion upper bound.
- The exporter writes HDF5 episode references plus lightweight future
  EEF/action windows; it still does not export RGB frames or run DINO.
...
- Task/suite conditioning alone was tested and is not sufficient; add visual
  grounding or action-aware objectives before treating learned future-motion
  priors as useful policy interfaces.
...
1. Build Gate 2.2: visual-grounded future-motion prior input on the same
   2-files-per-suite slice, starting with cached DINO features or a lightweight
   visual token if cache plumbing needs to be staged.
2. Add a stronger temporal predictor or action-aware auxiliary loss because
   task-conditioned MLP still fails to beat direct context.
3. Add a separate gripper/contact auxiliary target or diagnostic because Gate
   1.6 shows geometric action improves much more than gripper action.
4. For heavy training, use a GPU-visible shell or approved execution mode; the
   default restricted context sees `torch.cuda.is_available() == False`, while
   elevated Python sees the RTX 5090 correctly.
5. Add Gate-2 visual grounding probes for future EEF SE(3), geometric progress,
   and optional future DINO feature prediction.
6. Add `u_geom`, phase-bin, temporal-rank, and composition hard-negative
   diagnostics before treating any learned progress head as semantic.
7. Extend GeoMoCo-cVAE context with visual grounding token `g_t` and add
   deterministic AE, stochastic cVAE, direct residual, random, shuffled, and
   oracle future-motion baselines.
8. Before large cVAE training, compare direct BC with a controlled
   `GT future motion -> action decoder` upper bound.
9. Build the task-mining/headroom audit so mechanism tasks and LIBERO-Long /
