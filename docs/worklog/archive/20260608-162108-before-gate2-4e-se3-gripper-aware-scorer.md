...
- Next run target: turn the calibrated cVAE samples into a deployable readout
  with sample scoring / risk aggregation and gripper/contact diagnostics before
  making GeoMoCo-cVAE policy claims.
...
1. Implement Gate 2.4d sample scorer/readout for the calibrated free-bits cVAE:
   compare prior mean, random sample mean, oracle best-of-K, learned scorer
   argmax, and learned scorer soft aggregation.
2. Add a separate gripper/contact auxiliary target or diagnostic because Gate
