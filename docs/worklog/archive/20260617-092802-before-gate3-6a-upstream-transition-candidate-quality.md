...
1. Gate 3.5c is complete and not promoted. Predicted transition-probability
   gating is overall weak-positive (`0.033810` vs Gate 3.4 `0.034262`) but
   transition-negative (`0.133308` vs Gate 3.4 `0.131311`) and worsens
   transition MSE relative to the same frozen temporal base
...
2. Keep the immediate background: Gate 3.4b showed runtime K-sample usage,
   ...
   not transition, and Gate 3.5c showed deployable transition gating still does
   not repair close/open timing.
3. Next branch should move upstream to event/transition candidate quality
   rather than simply scaling residual adapters. Concrete options: calibrate
   transition confidence, allocate top-M/rank slots to transition candidates,
   or generate transition-specific event-conditioned candidates.
