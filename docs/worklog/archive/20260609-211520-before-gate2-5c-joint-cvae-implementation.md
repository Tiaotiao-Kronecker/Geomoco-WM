...
- Next run target: Gate 2.5c GeoMoCo-cVAE with joint
  `future_delta_ee + future_gripper/event` output, compared against the
  Gate 2.5b-joint deterministic baseline.
...
1. Implement Gate 2.5c: GeoMoCo-cVAE with joint
   `future_delta_ee + future_gripper/event` output.
2. Use Gate 2.5b-joint visual `lambda_action=0.300` as the deterministic
   baseline: action MSE `0.040688`, transition accuracy `0.560270`.
3. Keep task/proprio and shuffled-visual controls, and report event metrics in
   addition to action MSE and SE(3)/gripper metrics.
4. Revisit KL/free-bits/beta schedule after confirming the joint cVAE prior
   mean can at least match the deterministic joint baseline.
