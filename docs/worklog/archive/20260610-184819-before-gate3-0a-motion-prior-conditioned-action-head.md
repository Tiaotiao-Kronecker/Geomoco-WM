...
1. Start Gate 3.0: train a motion-prior-conditioned action head / planner that
   consumes the full set of sampled `future_delta_gripper` hypotheses from the
   frozen joint cVAE. Compare against direct context, prior mean, shuffled
   visual prior samples, and GT future-motion upper bound.
2. Reuse the existing joint `future_delta_gripper` cVAE and action decoder
   checkpoints; compare against the deterministic joint baseline `0.040688`.
3. Keep shuffled visual controls in every future readout table so the readout
   gain can be attributed to aligned visual grounding rather than dataset bias.
4. Add richer readout supervision before moving to multimodal action heads.
...
6. Extend GeoMoCo-cVAE context only after the current readout bottleneck is
   resolved.
