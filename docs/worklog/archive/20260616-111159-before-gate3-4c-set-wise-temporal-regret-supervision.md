...
- Next run target: Gate 3.4c set-wise temporal regret/rank supervision or a
  candidate-comparison auxiliary loss before moving to a small flow/diffusion
  residual decoder. Keep shuffled event metadata, rank/prob-only, trained
  mean_repeated, eval-time mean collapse, context-only/no-prior, permutation,
  and batch-mismatch controls.
...
1. Gate 3.4b usage audit is complete:
   ...
   Same-checkpoint eval-time mean collapse damages temporal MSE
   (`0.034218 -> 0.042594`), permutation is invariant (`action_l2=2.63e-7`),
   and batch mismatch is destructive (`0.314274`), so the full checkpoint uses
   aligned K-sample structure at runtime.
2. Keep the nuance: the separately trained Gate 3.4 mean_repeated control is
   still close (`0.034414` vs `0.034262`), so this is not yet a final
   diversity-attribution win. It means the next objective should make
   sample-distribution usage stronger under matched training controls.
3. Next branch: Gate 3.4c set-wise temporal regret/rank supervision or
   candidate-comparison auxiliary loss. Only after that should we move to a
   small flow/diffusion residual decoder, and only with the same attribution
   matrix.
4. Preserve repeated stochastic eval and group audit for the next decoder.
   Gate 3.2h showed oracle boundary masks are strong (`0.032018` overall MSE),
   Gate 3.2i showed boundary-index localization is still weak
   (`0.035053` boundary-index MSE), and Gate 3.3 showed a shallow additive
   gripper trajectory residual also fails (`0.035655` trajectory-routed MSE).
5. Do not add another deterministic boundary-routing variant unless it changes
   the action trajectory model itself; sparse CE, thresholding, positive-only
   residuals, oracle-mask diagnostics, and window-level index localization have
   all been tested.
6. For any future positive richer decoder, run attribution controls before
   promotion: shuffled event metadata, rank/prob-only, mean replacement,
   context-only/no prior, and same-capacity no-prior decoder.
