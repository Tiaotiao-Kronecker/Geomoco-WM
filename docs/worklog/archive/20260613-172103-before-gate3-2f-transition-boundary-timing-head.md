...
1. Start Gate 3.2f: step-wise transition-boundary timing using
   `close_step/open_step` from the event-mode audit JSON. Candidate first slice:
   predict `no_boundary / close_start / open_start` over the horizon and use it
   to gate a gripper residual.
...
3. Preserve repeated stochastic eval and group audit. The next question is
   whether boundary-start supervision can improve open/close transition windows
   without damaging sustain windows.
