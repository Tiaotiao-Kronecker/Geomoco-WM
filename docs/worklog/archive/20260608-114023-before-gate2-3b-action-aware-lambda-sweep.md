...
- Next run target: sweep action-aware loss weight, add multi-query visual
  attention, and add gripper/contact auxiliary modeling before promoting to
  GeoMoCo-cVAE.
...
- Task/suite conditioning alone was tested and is not sufficient; DINO global
  ...
  pass. Gate 2.3a action-aware training is positive; the next blocker is
  whether lambda sweep, multi-query attention, and gripper/contact modeling can
...
1. Sweep action-aware loss weight around `0.003`, `0.01`, and `0.03` using the
   aligned two-camera DINO patch cross-attention branch.
2. Add a stronger temporal or multi-query visual predictor so different future
