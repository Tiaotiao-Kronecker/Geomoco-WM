...
- Gate 2.4h-c cVAE sample event-alignment analysis completed; event oracle
  best-of-K improves event accuracy from `0.837121` to `0.853438`, but
  transition step-within-1 coverage reaches only `0.233850` and flat ScoreNet
  reaches `0.170543`, so current samples/readout only weakly capture
  close/open timing.
...
- Next run target: Gate 2.4h-d minimal event-aware ScoreNet diagnostic,
  testing whether weak transition-type/timing supervision can improve event
  alignment without regressing deployable action MSE.
...
1. Implement Gate 2.4h-c: cVAE sample event-alignment analysis using the
   Gate 2.4h-a transition labels and Gate 2.4c/2.4d sample readout pipeline.
2. Keep Gate 2.4d flat action-MSE ScoreNet as the current deployable readout
   baseline; use Gate 2.4f `SE(3)` / `SE(3)+gripper` ranks and Gate 2.4g
   hard-negative results as diagnostics.
3. Add `u_geom`, phase-bin, temporal-rank, and composition hard-negative
   diagnostics before treating any learned progress head as semantic.
4. Prototype a GuidedVLA-style zero-initialized guidance branch or lightweight
   factor adapter only after the scorer/readout gate is interpretable.
...
6. Extend GeoMoCo-cVAE context with visual grounding token `g_t` and add
   deterministic AE, stochastic cVAE, direct residual, random, shuffled, and
   oracle future-motion baselines.
7. Before large cVAE training, compare direct BC with a controlled
   `GT future motion -> action decoder` upper bound.
8. Build the task-mining/headroom audit so mechanism tasks and LIBERO-Long /
