...
1. Start Gate 3.2d: transition-gated residual action head or event-routed
   output layer. Candidate first slice: keep shared SE(3) regression but add a
   small event-family/transition gate that specializes gripper residuals.
...
3. Preserve repeated stochastic eval and group audit. The next question is
   whether explicit routing/residual specialization can improve
   transition/open-close windows without damaging sustain windows.
