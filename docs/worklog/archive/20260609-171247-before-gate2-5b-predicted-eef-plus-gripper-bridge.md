...
- Next run target: Gate 2.5b predicted future EEF + predicted future gripper,
  removing the remaining GT future-EEF privilege from the Gate 2.5a bridge
  diagnostic.
...
1. Implement Gate 2.5b: evaluate predicted future EEF + predicted future
   gripper through the Gate 2.4i `future_delta_gripper` action decoder.
2. Start with a modular bridge using the best existing visual EEF prior plus
   the Gate 2.5a visual gripper predictor, then compare against a jointly
   trained `future_delta_gripper` predictor if the modular bridge is positive.
3. Keep task/proprio and shuffled-visual controls so any 2.5b gain is still
   attributable to aligned visual grounding.
4. If Gate 2.5b improves over the EEF-only learned prior, promote Gate 2.5c:
