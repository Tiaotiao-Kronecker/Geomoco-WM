...
- Next run target: Gate 3.2g boundary-quality audit plus transition-local
  repair before moving to flow matching / diffusion action heads.
...
1. Start Gate 3.2g: boundary-quality audit plus transition-local repair.
   First report boundary precision/recall or PR-AUC for
   `no_boundary / close_start / open_start`; then test a calibrated or
   transition-local gripper residual that does not perturb sustain windows.
2. Keep Gate 3.1f/g full event/rank/prob top-4 as the current deployable
   ...
   reproduces this with action MSE `0.034773`. Gate 3.2f does not beat it.
3. Preserve repeated stochastic eval and group audit. Gate 3.2f showed a small
   transition gain but worsened overall/gripper/sustain metrics, so the next
   question is whether boundary confidence can localize residual application
   without moving error into sustain windows.
4. Do not jump to flow matching yet. First isolate the transition-timing
   bottleneck under the deterministic event-aware action-head protocol.
