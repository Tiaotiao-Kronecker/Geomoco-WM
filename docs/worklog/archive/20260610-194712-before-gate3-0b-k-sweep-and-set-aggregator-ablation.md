...
1. Continue Gate 3.0 from the positive Gate 3.0a result. The next clean
   ablation is a K sweep (`4/8/16/32`) and/or set-aggregator comparison while
   keeping raw DINO out of the action head.
2. Preserve the Gate 3.0a evaluation contract: compare context-only, prior
   mean, real sample set, shuffled sample set, and GT future under the same
   downstream action-head protocol.
3. Do not mix Gate 3.0a action-head metrics with old frozen-action-decoder
   metrics; use them as separate evidence streams.
4. Keep repeated stochastic eval for sample-set modes so single sampling noise
   does not drive promotion decisions.
