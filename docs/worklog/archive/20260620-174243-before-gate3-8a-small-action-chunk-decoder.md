...
4. Gate 3.7a minimal soft event-time latent + conditional temporal decoder has
   ...
   event-time latent.
5. Preserve the attribution ledger for any next positive decoder or candidate
   branch:
...
6. Do not mix Gate 3 action-head metrics with old frozen-action-decoder metrics;
   use them as separate evidence streams.
7. For heavy training, use a GPU-visible shell or approved execution mode; the
