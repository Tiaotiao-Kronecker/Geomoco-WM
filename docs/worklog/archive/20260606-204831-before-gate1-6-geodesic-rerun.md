...
- Gate 1.6 two-file four-suite oracle action-decoder diagnostic completed.
...
- Next run target: either rerun the 2-files-per-suite oracle gate with the new
  geodesic metrics or build the first learned future-motion prior and evaluate
  it between the direct-context lower bound and oracle-future-motion upper
  bound.
...
- Decide whether to rerun Gate 1.6 immediately with geodesic metrics for a
  clean replacement table, or keep Gate 1.6 as the normalized/split-metric
  scale-up and use geodesic metrics from the next learned-prior gate onward.

## Next Session Entry Point
1. Optionally rerun the 2-files-per-suite oracle action-decoder gate with the
   new physical/geodesic metrics for a clean apples-to-apples table.
2. Build the first learned future-motion prior on the 2-files-per-suite window
   slice: context/proprio -> future EEF delta.
3. Evaluate the learned prior through the same action-decoder interface against
   direct-context and oracle-future-motion bounds, reporting flat and
   physical/geodesic action metrics.
