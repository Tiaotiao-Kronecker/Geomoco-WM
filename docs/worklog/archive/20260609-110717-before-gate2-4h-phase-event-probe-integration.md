...
- Next run target: event/executability-aware readout target construction,
  starting with gripper-transition and contact-proxy labels from existing
  action/proprio streams.
...
1. Implement the next readout target builder: gripper-transition,
   contact-proxy, and event-timing labels from existing action/proprio streams.
