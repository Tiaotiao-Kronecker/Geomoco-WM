...
- Next run target: add action-aware or multimodal future-motion modeling to
  reduce the remaining Gate-2.2b-to-oracle gap before promoting to GeoMoCo-cVAE.
...
- Task/suite conditioning alone was tested and is not sufficient; DINO global
  ...
  pass. The next blocker is reducing the remaining oracle gap with
  action-aware or multimodal modeling before cVAE claims.
...
1. Add an action-aware auxiliary loss or frozen-action-decoder loss on top of
   aligned two-camera DINO patch cross-attention.
