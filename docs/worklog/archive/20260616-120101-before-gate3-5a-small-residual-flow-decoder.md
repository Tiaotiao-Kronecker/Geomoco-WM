...
1. Gate 3.4c is complete and not promoted:
   `docs/experiments/runs/2026-06-16_gate3_4c_sample_score_temporal_regret.md`
   and
   `docs/experiments/comparisons/2026-06-16_gate3_4c_sample_score_temporal_regret_summary.md`.
   Full-aligned repeated eval over seeds 7/17 gives temporal MSE `0.034779`,
   worse than Gate 3.4 full aligned `0.034262`, so do not expand the full
   Gate 3.4c control matrix.
2. Keep the attribution nuance from Gate 3.4b:
   same-checkpoint mean collapse damages Gate 3.4 (`0.034218 -> 0.042594`),
   but separately trained mean_repeated remains close (`0.034414` vs
   `0.034262`). Samples are used at runtime; training still has not made
   diversity irreducible under matched controls.
3. Next branch should be a small flow/diffusion residual decoder, or the
   smallest flow-like residual trajectory decoder, not another shallow
   deterministic boundary-routing variant. Gate 3.2h showed oracle boundary
   masks are strong (`0.032018` overall MSE), Gate 3.2i showed boundary-index
   localization is weak (`0.035053` boundary-index MSE), Gate 3.3 showed a
   shallow gripper trajectory residual fails (`0.035655`), and Gate 3.4c showed
   motion-regret sample scoring is not enough.
4. Preserve the attribution ledger for the next richer decoder:
...
5. Run full controls only after the full-aligned richer decoder first beats
