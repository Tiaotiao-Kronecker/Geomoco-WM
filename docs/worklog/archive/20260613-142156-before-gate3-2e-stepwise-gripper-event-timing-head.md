...
- Next run target: Gate 3.2d transition-gated residual action head or
  event-routed output layer before moving to flow matching / diffusion action
  heads.
...
1. Start Gate 3.2e: step-wise gripper/event-timing head or temporal transition
   mask. Candidate first slice: keep the Gate 3.1f/g top-4 event/rank/prob
   interface, but predict per-step gripper transition logits or a temporal
   residual mask instead of one window-level event-family residual.
2. Keep Gate 3.1f/g full event/rank/prob top-4 as the current deployable
   ...
   reproduces this with action MSE `0.034773`.
3. Preserve repeated stochastic eval and group audit. The next question is
   whether step-wise timing supervision can improve open/close transition
   windows without damaging sustain windows.
