...
7. Flow eval-only calibration and dataset audit now show that more Euler steps
   do not fix flow MSE, and the current 2-files-per-suite slice is sparse for
   the actual bottleneck: only `10.29%` transition windows, `878` close
   transition windows, and `807` open transition windows. The next practical
   step should be dataset expansion and/or a transition-balanced sampler before
   spending more compute on strong-policy replacement controls.
8. Preserve the attribution ledger for any next positive decoder or candidate
   branch:
...
9. Do not mix Gate 3 action-head metrics with old frozen-action-decoder metrics;
   use them as separate evidence streams.
10. For heavy training, use a GPU-visible shell or approved execution mode; the
