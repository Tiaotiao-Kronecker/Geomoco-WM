...
- Next run target: Gate 3.2h oracle-boundary transition-local gripper
  correction, or formally pivot to a richer temporal/flow action head after
...
1. Start Gate 3.2h: oracle-boundary transition-local gripper correction.
   First use `close_step/open_step` as an oracle mask to train/apply a local
   gripper correction only near true boundaries; only build a predicted-mask
   version if the oracle-mask upper bound beats Gate 3.1f/Gate 3.1g without
   damaging sustain windows.
2. Keep Gate 3.1f/g full event/rank/prob top-4 as the current deployable
   ...
   reproduces this with action MSE `0.034773`. Gate 3.2f/g do not beat it.
3. Preserve repeated stochastic eval, group audit, and boundary-quality audit.
   Gate 3.2g showed that positive-only residuals can slightly improve
   transition MSE, but CE-weighted boundary prediction trades recall for too
   many false positives.
4. Do not jump to flow matching yet. First isolate the transition-timing
   bottleneck under the deterministic event-aware action-head protocol with an
