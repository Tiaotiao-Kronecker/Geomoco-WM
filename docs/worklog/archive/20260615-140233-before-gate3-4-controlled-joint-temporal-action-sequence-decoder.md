...
- Next run target: Gate 3.3 short-budget temporal gripper trajectory run. Use
  the fixed Gate 3.1f/g predicted top-4 `event_rank_prob` interface and compare
  `trajectory_routed_*` against base, transition/sustain groups, and later the
  metadata/no-prior controls before making any promotion claim.
...
1. Gate 3.3 `temporal_mlp` is complete and negative. Decide the next branch:
   a controlled joint temporal action-sequence decoder, a small flow/diffusion
   action residual decoder, or richer contact/object/event supervision before
   adding more decoder capacity.
2. Keep Gate 3.1f/g full event/rank/prob top-4 as the current deployable
   ...
   reproduces this with action MSE `0.034773`. Gate 3.2f/g/h/i do not beat it
   as deployable predicted boundary/localizer heads.
3. Preserve repeated stochastic eval and group audit for the next decoder.
...
4. Do not add another deterministic boundary-routing variant unless it changes
   ...
   all been tested.
5. For any future positive richer decoder, run attribution controls before
