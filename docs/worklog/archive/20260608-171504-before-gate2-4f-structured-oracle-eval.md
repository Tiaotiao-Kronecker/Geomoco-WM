...
- Next run target: strengthen the cVAE sample readout with hard-negative
  ranking or richer gripper/contact/executability proxies before making
  GeoMoCo-cVAE policy claims.
...
1. Implement the next readout branch after Gate 2.4e: hard-negative ranking or
   richer gripper/contact/executability proxies over calibrated cVAE samples.
2. Keep Gate 2.4d flat action-MSE ScoreNet as the current deployable readout
   baseline; use Gate 2.4e as a negative ablation for naive structured target
   replacement.
