...
- Next run target: Gate 2.5d joint cVAE sample readout/scorer, converting the
  best-of-K action MSE `0.022139` into a deployable readout against the
...
1. Implement Gate 2.5d: joint cVAE sample readout/scorer for
   `future_delta_gripper` samples.
2. Use Gate 2.5c real visual cVAE `prior_recon_weight=0.5` as the cVAE
   checkpoint; compare prior mean `0.043816`, deterministic joint `0.040688`,
   and best-of-K `0.022139`.
3. Start with the existing lightweight ScoreNet/readout path, but update any
   EEF-only assumptions to `future_delta_gripper` metrics and gripper-event
   evaluation.
4. Keep shuffled visual controls and event fidelity in the readout table; a
   readout that only improves flat action MSE while hurting transition timing
