...
11. Do not treat full-data DINO/cache/model retraining as the next automatic
   move. The cleaner next step is Gate 3.10d: audit whether the current
   gripper-command transition labels are missing contact/object-state/task-
   phase information. If the goal shifts to pure engineering performance, use
   transition-focused context-only/action policy as the strong baseline and
   reposition GeoMoCo-WM as geometry/event auxiliary rather than the primary
   transition decoder.
12. Preserve the attribution ledger for any next positive decoder or candidate
   branch:
...
13. Do not mix Gate 3 action-head metrics with old frozen-action-decoder metrics;
   use them as separate evidence streams.
14. For heavy training, use a GPU-visible shell or approved execution mode; the
