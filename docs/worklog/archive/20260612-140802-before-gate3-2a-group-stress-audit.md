...
- Next run target: Gate 3.2 stress-test or scale the promoted event-aware
  top-4 interface before moving to flow matching / diffusion action heads.
...
1. Start Gate 3.2: stress-test or scale the promoted event-aware top-4
   interface. Candidate first slice: run the same interface on a larger
   files-per-suite slice or a transition-heavy / LIBERO-10-heavy subset.
2. Keep Gate 3.1f/g full event/rank/prob top-4 as the current deployable
   reference: action MSE `0.034767`, gripper MSE `0.150052`.
3. Preserve repeated stochastic eval and controls. The next question is whether
   the mechanism survives more data/harder tasks, not whether top-M metadata
   helps on the current slice.
4. Do not jump to flow matching yet. First establish robustness of the
   deterministic event-aware action-head interface.
