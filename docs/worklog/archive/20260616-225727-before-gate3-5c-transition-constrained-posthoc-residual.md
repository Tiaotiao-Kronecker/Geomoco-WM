...
1. Gate 3.5b is complete and not promoted as the default. It is
   overall-positive (`0.033374` vs Gate 3.4 `0.034262`) but
   transition-negative (`0.133339` vs Gate 3.4 `0.131311`) and
   attribution-thin (`prior gain=+0.000354`, `diversity gain=+0.000230`).
2. Keep the immediate background: Gate 3.4b showed runtime K-sample usage,
   ...
   training regressed, and Gate 3.5b post-hoc residual capacity helped overall
   but did not fix transition/gripper timing.
3. Next branch should not simply scale adapter capacity. It should either
   constrain residual adaptation to transition-local errors with deployable
   event confidence, or improve upstream event/transition candidate quality,
   while preserving decoder/prior/metadata/diversity controls.
4. Preserve the attribution ledger for any next positive decoder:
