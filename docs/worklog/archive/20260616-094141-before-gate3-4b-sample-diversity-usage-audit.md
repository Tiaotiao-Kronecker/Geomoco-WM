...
- Next run target: decide whether to strengthen Gate 3.4 sample-diversity use
  with set-wise temporal/regret supervision, or move to a small flow/diffusion
  residual decoder under the same attribution controls. Do not promote a
  richer decoder without beating shuffled event metadata, rank/prob-only, mean
  replacement, and same-capacity no-prior controls.
...
1. Gate 3.4 `sequence_mlp` joint temporal action decoder is complete as a
   small positive: full aligned temporal-action MSE `0.034262` and transition
   MSE `0.131311`, versus Gate 3.1f/g reference `0.034767` and `0.134087`.
   Its main caveat is mean replacement staying close at `0.034414`, so
   sample-diversity attribution is still thin.
2. Decide the next branch: strengthen sample-diversity usage with set-wise
   temporal/regret/rank supervision, or test a small flow/diffusion residual
   decoder under the same controls. Avoid another shallow additive gripper-only
   branch.
3. Keep Gate 3.4 as the current short-budget mechanism-positive reference, but
   keep Gate 3.1f/g as the cleaner deployable baseline when strict attribution
   is more important than the small numeric gain.
