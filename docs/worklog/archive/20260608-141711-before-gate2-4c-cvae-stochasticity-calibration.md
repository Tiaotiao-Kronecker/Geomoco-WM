...
- Next run target: calibrate cVAE stochasticity with prior sampling,
  best-of-K/coverage metrics, KL/free-bits or beta schedules, and
  gripper/contact diagnostics before making GeoMoCo-cVAE policy claims.
...
1. Add cVAE prior sampling, best-of-K future-motion coverage metrics, and
   KL/free-bits or beta-schedule diagnostics.
...
3. For heavy training, use a GPU-visible shell or approved execution mode; the
   default restricted context sees `torch.cuda.is_available() == False`, while
   elevated Python sees the RTX 5090 correctly.
4. Add Gate-2 visual grounding probes for future EEF SE(3), geometric progress,
