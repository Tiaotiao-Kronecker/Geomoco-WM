...
1. Start the pivot after Gate 3.2i: design the smallest richer temporal/flow
   action decoder or direct gripper-transition trajectory model that can handle
   close/open shape without relying on a brittle deterministic boundary mask.
...
3. Preserve repeated stochastic eval and group audit for the next decoder.
   ...
   while Gate 3.2i showed a different index objective still fails to make
   predicted timing deployable (`0.035053` boundary-index MSE).
