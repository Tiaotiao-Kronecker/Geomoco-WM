...
- Full four-suite export/audit is now complete for `horizon=8`, `stride=4`.
  The next full-data blocker is not raw window export or CI plumbing; it is
  deciding whether to build full-data DINO/model artifacts now, given that
  Gate 3.10b found geometry/metadata gains but not reliable transition/open-
  close action-timing gains on the existing slice.
...
10. Do not treat full-data DINO/cache/model retraining as an automatic next
   step. The next clean choice is either:
   (a) run a transition-balanced or transition-stratified small training audit
   on the existing slice to test whether timing gains become reliable, or
   (b) explicitly spend full-data compute to test whether more transition data
   alone fixes the CI instability. If choosing (b), rerun only the key
   Gate 3.4-style controls first.
11. Preserve the attribution ledger for any next positive decoder or candidate
   branch:
...
12. Do not mix Gate 3 action-head metrics with old frozen-action-decoder metrics;
   use them as separate evidence streams.
13. For heavy training, use a GPU-visible shell or approved execution mode; the
