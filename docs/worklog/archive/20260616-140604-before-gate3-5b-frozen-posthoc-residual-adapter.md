...
1. Gate 3.5a is complete and not promoted. The jointly trained residual flow
   readout has mean MSE `0.035621`, worse than Gate 3.4 `0.034262`, and worse
   than its own temporal readout `0.035392`. Do not spend the full controls on
   this branch.
2. Next branch should be Gate 3.5b: freeze the promoted Gate 3.4 checkpoint and
   train a post-hoc residual adapter on top of frozen `temporal_actions`. This
   isolates residual action modeling from shared-representation degradation.
3. Keep the immediate background: Gate 3.4c motion-regret scoring also failed
   (`0.034779` vs Gate 3.4 `0.034262`), while Gate 3.4b showed samples are used
   at runtime but trained mean_repeated remains close. The current bottleneck is
   not "samples ignored"; it is making residual/action improvement without
   losing attribution or damaging the temporal decoder.
4. Preserve the attribution ledger for any positive residual adapter:
   decoder gain = same-checkpoint base minus richer decoder;
...
5. Run full controls only after the full-aligned residual adapter first beats
   Gate 3.4. Required controls remain shuffled event metadata, rank/prob-only,
   trained mean_repeated, context-only/no-prior, eval-time mean collapse,
   permutation invariance, and batch mismatch.
