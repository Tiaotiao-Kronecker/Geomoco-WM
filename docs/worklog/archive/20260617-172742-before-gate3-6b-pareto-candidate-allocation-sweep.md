...
- Gate 3.6a upstream transition-reserve candidate quality completed as a
  controlled mechanism-positive but not promoted result:
  plan at
  ...
  The new `transition_reserve` event candidate policy improves temporal
  transition MSE from Gate 3.4 `0.131311` to `0.126421`, and controls support
  attribution: context-only `0.131626`, shuffled metadata `0.127638`,
  rank/prob-only `0.128118`, and mean-repeated `0.127759` are all worse on
  transition. It is not promoted as the default because overall temporal MSE
  regresses (`0.034262 -> 0.036391`) and sustain MSE regresses
  (`0.022542 -> 0.025539`). The next upstream step should make transition
  candidate allocation Pareto-aware rather than use a fixed reserve rule.
...
1. Gate 3.6a is complete and not promoted as default. It is transition-positive
   (`0.126421` vs Gate 3.4 `0.131311`) with attribution controls, but
   overall/sustain-negative (`temporal MSE 0.036391` vs `0.034262`, sustain
   `0.025539` vs `0.022542`).
2. Keep the immediate background: Gate 3.4b showed runtime K-sample usage,
   ...
   repair close/open timing, and Gate 3.6a showed upstream transition candidate
   allocation is the right lever but creates an overall/sustain trade-off.
3. Next branch should stay upstream and make transition candidate allocation
   Pareto-aware. Concrete options: tune/calibrate transition-reserve threshold,
   reserve transition slots only under high-risk uncertainty, or score whether
   reserved transition candidates actually reduce action regret before they are
   forced into the top-M set.
4. Preserve the attribution ledger for any next positive decoder or candidate
   branch:
