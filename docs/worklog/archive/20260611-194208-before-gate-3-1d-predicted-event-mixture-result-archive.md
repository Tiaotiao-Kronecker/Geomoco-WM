...
- Next run target: Gate 3.1d predicted event-mode / top-M mixture sampling,
  using the Gate 3.1b event predictor idea to replace oracle event labels.
...
1. Start Gate 3.1: mode-structured / event-aware future-motion prior. The
   immediate design question is how to expose gripper-transition/event timing as
   an explicit mode variable or diagnostic without overfitting to labels.
...
3. Preserve real-vs-shuffled sample-set controls, repeated stochastic eval, and
   Gate 3.0c usage-audit probes for any new prior.
4. Focus next on aligned diversity, especially transition-heavy windows and
   LIBERO-10 long-horizon tasks. Do not optimize for generic sample variance.
