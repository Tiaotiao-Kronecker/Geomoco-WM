...
1. Start Gate 3.0c: sample-set mode/diversity and action-head usage audit.
   Check whether cVAE samples expose distinct action-useful modes and whether
   the action head uses them or collapses them to a mean statistic.
2. Keep Gate 3.0 defaults as `context_attention`, K=16. Use K=32 only as an
   optional stronger reference because its mean MSE gain is tiny.
3. Preserve real-vs-shuffled sample-set controls and repeated stochastic eval.
4. Suggested Gate 3.0c probes: sample permutation/dropout/mean replacement,
   action-output sensitivity to sample changes, per-task/window benefit from
   K=32, and correlation with gripper-transition or LIBERO-10 long-horizon
   windows.
