...
- Next run target: Gate 2.5b-joint deterministic `future_delta_gripper`
  predictor with visual grounding and optional action-aware loss through the
...
1. Implement Gate 2.5b-joint: train a deterministic
   `future_delta_gripper` predictor using the same two-file four-suite slice.
2. Start with the real visual patchpool branch, action-aware loss weight
   `0.030`, and the Gate 2.4i `future_delta_gripper` action decoder; compare
   against the modular bridge action MSE `0.050333` and best EEF-only learned
   prior `0.042090`.
3. If the real joint branch is positive, add task/proprio and shuffled-visual
   controls so the gain remains attributable to aligned visual grounding.
4. If Gate 2.5b-joint improves over the EEF-only learned prior, promote Gate
   2.5c:
   GeoMoCo-cVAE with joint EEF+gripper/event output.
5. Add `u_geom`, phase-bin, temporal-rank, and composition hard-negative
   diagnostics before treating any learned progress head as semantic.
6. Prototype a GuidedVLA-style zero-initialized guidance branch or lightweight
