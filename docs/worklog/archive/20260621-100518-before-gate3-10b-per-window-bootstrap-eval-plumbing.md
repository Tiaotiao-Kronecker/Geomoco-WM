...
- Gate 3.10b reliability tooling started and the generic episode-bootstrap CI
  utility is implemented:
  `scripts/bootstrap_episode_ci.py` with tests in
  `tests/test_bootstrap_episode_ci.py`. It compares aligned per-window metric
  artifacts by resampling validation episodes with replacement and reports
  observed gain, bootstrap mean gain, 95% CI, and whether the CI crosses zero.
  It is ready for Gate 3.4/Gate 3.9 reruns once the relevant evaluators emit
  per-window metrics.
...
- Full four-suite export/audit is now complete for `horizon=8`, `stride=4`.
  The next full-data blocker is not raw window export; it is building the
  full-data DINO feature cache and making key model evaluators emit aligned
  per-window metrics for episode-bootstrap CI.
...
8. Gate 3.10a full LIBERO audit now changes the data status: full data has
   ...
   claims.
9. Gate 3.10b bootstrap CI tooling exists as a generic script, but model
   evaluators still need per-window metric export before old claims can be
   retested with confidence intervals.
10. The next practical implementation step is a reliability rerun substrate:
   add per-window metric export to the key Gate 3.4/Gate 3.9 evaluators, then
   build full-data DINO cache/artifacts and rerun only the essential controls.
