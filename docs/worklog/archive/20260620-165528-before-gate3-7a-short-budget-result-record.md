...
- Gate 3.7a plumbing implemented and smoke-tested:
  ...
  Verification passed: compileall, ruff, and 69 unittest cases. Formal
  seed-7/17 GPU runs are still pending.
...
4. The chosen next branch is Gate 3.7a: minimal soft event-time latent +
   conditional temporal decoder. It should keep the Gate 3.4 top-k
   event/rank/prob sample interface and select by `temporal_action_mse`, not
   by transition-only MSE. The implementation and CPU smoke are complete; next
   run the two-seed GPU short-budget full-aligned experiment.
