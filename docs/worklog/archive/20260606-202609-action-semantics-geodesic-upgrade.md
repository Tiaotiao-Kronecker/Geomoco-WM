...
- Gate 1.6 two-file four-suite oracle action-decoder diagnostic completed with
  SE(3)-aware action metrics.
- Next run target: build the first learned future-motion prior and evaluate it
  between the direct-context lower bound and oracle-future-motion upper bound.
...
1. Build the first learned future-motion prior on the 2-files-per-suite window
   slice: context/proprio -> future EEF delta.
2. Evaluate the learned prior through the same action-decoder interface against
   direct-context and oracle-future-motion bounds, reporting flat and
   SE(3)-aware metrics.
